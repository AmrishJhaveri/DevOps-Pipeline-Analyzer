...
const CONSTANTS = {
  JENKINS_FILE_INFO_WITH_REPO: './app/q4/finalOutput.json',
...
function processEachStageBlock() {
  return async function(eachStageObj) {
    ...
    // let count = parseBasedOnOutput.parallel_stages_analysis[stageName];
    // if (!parseBasedOnOutput.parallel_stages_analysis[stageName]) {
    //   parseBasedOnOutput.parallel_stages_analysis[stageName] = 1;
    // } else {
    //   parseBasedOnOutput.parallel_stages_analysis[stageName] = count + 1;
    // }
...
function writeIntermediateFile() {
  fs.writeFileSync(
      ...
      JSON.stringify(
          parseBasedOnOutput.parallel_stages_analysis, undefined, 2));
}
